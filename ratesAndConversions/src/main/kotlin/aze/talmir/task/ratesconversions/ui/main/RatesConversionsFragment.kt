...
class RatesConversionsFragment : Fragment() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        val adapter = RatesConversionsDataAdapter(currencyDataItemClick, coefficientChangeListener, focusChangeListener)
        ...
        ratesConversionsData.observe(viewLifecycleOwner, Observer { result ->
            when (result) {
                is Result.Success -> adapter.submitList(result.data.toList())
                is Result.Error -> println("ERROR: ${result.msg}")
                else -> println("basqa zibildi... $result")
            }
        })
    }
